...
function buildPrerequisites(spec) {
  ...
  lines.push('');
  lines.push('**None of the scripts prompt you for these paths anymore.** Every script reads');
  lines.push('them from `lab-config.json`, which must sit in the same folder as the scripts.');
  lines.push('Copy `lab-config.json.example` (included in this output) to `lab-config.json`');
  lines.push('and fill in the paths below before running anything. Each script validates that');
  lines.push('its file actually exists at the configured path and stops with a clear error if not.');
  lines.push('');
  ...
  lines.push('│                         lab-config.json (copied from lab-config.json.example)');
  lines.push('└── Output\\             ← generated output lands here');
  lines.push('```');
  lines.push('');
  lines.push('With that layout, `lab-config.json` (in `Scripts\\`, next to the scripts) looks like:');
  lines.push('');
  lines.push('```json');
  lines.push('{');
