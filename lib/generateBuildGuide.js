...
function buildBuildGuide(spec) {
  ...
  if (dc.enabled) {
    ...
    out.push('```');
    out.push('');
    out.push(`Creates the \`lab-dc-01\` VM shell (${dcVcpu} vCPU / ${dcRamGB}GB RAM / ${dcDiskGB}GB OS disk${dcProfile === 'dc-jumpbox-fileserver' ? ` + ${dcStorageDiskGB}GB storage disk` : ''}) and attaches the Windows Server ISO. Profile: **${dcProfileLabel}**.`);
    out.push('');
  ...
  if (isOva) {
    ...
    out.push('    -EsxiIsoPath "[datastore] ISOs/esxi.iso" `');
    out.push('    -WizardIp <ip-of-this-machine>   # optional — prints kickstart URL per host');
    out.push('```');
    out.push('');
    out.push(`Creates ${nc.hostCount} nested VM shell${nc.hostCount === 1 ? '' : 's'} with port groups, then powers each VM on immediately.`);
