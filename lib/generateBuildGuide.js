...
function buildBuildGuide(spec) {
  ...
  if (spec.learningMode) {
    ...
    if (spec.useCase === 'certification') {
      ...
      out.push('');
      const objectives = ['VCP-VVF — ESXi and vCenter deployment and configuration'];
      if (nc.vsanEnabled) objectives.push('VCP-VVF — vSAN cluster configuration and management');
      if (nsx.enabled)    objectives.push('VCP-NV — NSX deployment, segments, and gateway routing');
      if (nsx.bgpEnabled) objectives.push('VCAP-NV — dynamic routing and BGP peering');
      if (spec.vcf && spec.vcf.enabled) objectives.push('VCP-VCF Administrator — management domain bring-up and SDDC Manager operations');
