...
const DC_VCPU_BY_PROFILE    = { 'none': 0, 'dc-only': 2, 'dc-jumpbox': 4, 'dc-jumpbox-fileserver': 4 };
const DC_VRAM_GB_BY_PROFILE = { 'none': 0, 'dc-only': 4, 'dc-jumpbox': 8, 'dc-jumpbox-fileserver': 8 };
const DEPOT_VCPU = 2;
...
function evaluateSizing(answers) {
  ...
  const dcEnabled = dcProfile !== 'none';
  ...
  if (applianceVramGB > 0) {
    const dcLabel = dcProfile === 'dc-jumpbox' ? 'DC+Jumpbox' : dcProfile === 'dc-jumpbox-fileserver' ? 'DC+Jumpbox+FS' : dcEnabled ? 'DC' : null;
  ...
  if (physicalStorageGB > 0 && nestedHostCount > 0 && (nestedDisks.length > 0 || memTieringEnabled)) {
    ...
    const totalVirtualGB =
      ...
      depotDiskAlloc + dcStorageDiskAlloc;
    if (totalVirtualGB > physicalStorageGB) {
      warnings.push(
        `Virtual disk allocation across all nested hosts and appliances (${totalVirtualGB}GB total${depotDiskAlloc ? `, including ${depotDiskAlloc}GB for depot VM` : ''}${dcStorageDiskAlloc ? `, including ${dcStorageDiskAlloc}GB for DC storage disk` : ''}) exceeds total physical storage ` +
...
module.exports = { evaluateSizing, recommendHostTiers };
