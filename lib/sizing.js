...
function evaluateSizing(answers) {
  ...
  const applianceVcpu = (vyosEnabled ? VYOS_VCPU : 0) + (dcEnabled ? DC_VCPU : 0) + (depotLinux ? DEPOT_VCPU : 0);
  const applianceVramGB = (vyosEnabled ? VYOS_VRAM_GB : 0) + (dcEnabled ? DC_VRAM_GB : 0) + (depotLinux ? DEPOT_VRAM_GB : 0);
  ...
  if (applianceVramGB > 0) {
    const names = [vyosEnabled && 'VyOS', dcEnabled && 'DC', depotLinux && 'depot VM'].filter(Boolean).join(', ');
