...
function buildDeployLabOva(spec) {
  ...
  lines.push('    Detects govc and uses it for the OVA import if available (falls back to PowerCLI');
  lines.push('    Import-VApp otherwise) -- Import-VApp has known reliability problems against a');
  lines.push('    standalone ESXi host, since vApp import assumes a vCenter-managed inventory.');
  lines.push('    govc has no NVMe controller support, so ESA storage-pool disks are still attached');
  lines.push('    via PowerCLI regardless of which import path created the VM.');
  lines.push('');
  lines.push('    Each nested host gets ONE vNIC, on Nested-Trunk (VLAN 4095 trunk on vSwitch1).');
  ...
  lines.push('Set-PowerCLIConfiguration -InvalidCertificateAction Ignore -Confirm:$false | Out-Null');
  lines.push('');
  lines.push('# PowerCLI is still needed either way (vSwitch/port group setup, host/datastore');
  lines.push('# lookup, post-import reconfiguration). govc is only used for the OVA import itself --');
  lines.push('# Import-VApp/Get-OvfConfiguration has known reliability problems against a standalone');
  lines.push('# ESXi host (vApp import semantics assume a vCenter-managed inventory), which govc');
  lines.push('# does not depend on.');
  lines.push('$govcAvailable = Get-Command govc -ErrorAction SilentlyContinue');
  lines.push('if ($govcAvailable) {');
  lines.push('    Write-Host "govc found -- using govc for OVA import"');
  lines.push('} else {');
  lines.push('    Write-Host "govc not found -- using PowerCLI Import-VApp. For more reliable OVA import"');
  lines.push('    Write-Host "against a standalone ESXi host, install govc: https://github.com/vmware/govmomi/releases"');
  lines.push('}');
  lines.push('');
  ...
  const emitOvaImport = (lines, indent = '', hostRef) => {
    ...
    lines.push('');
    lines.push(`${indent}if ($govcAvailable) {`);
