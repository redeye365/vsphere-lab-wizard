...
function buildVyosDeploy(spec) {
  ...
  lines.push('');
  lines.push('# VM folders require vCenter -- a standalone ESXi host has only the flat');
  lines.push('# default "vm" folder and New-Folder throws against it. Skip gracefully and');
  lines.push('# deploy without a folder rather than aborting the whole script.');
  lines.push('$vmFolderAvailable = $true');
  lines.push('try {');
  ...
  lines.push('    }');
  lines.push('} catch {');
  lines.push('    Write-Warning "Could not create VM folder \'$VmFolder\' -- this is expected when connecting directly to a standalone ESXi host (VM folders require vCenter). Continuing without a folder."');
  lines.push('    $vmFolderAvailable = $false');
  lines.push('}');
  lines.push('');
  ...
  lines.push('    CD        = $true');
  lines.push('}');
  lines.push('if ($vmFolderAvailable) { $newVmParams.Location = $VmFolder }');
  lines.push('$vm = New-VM @newVmParams');
  lines.push('');
...
// --- Helper: emit port-group creation block (reused across single/multi-host paths) ---

function emitPortGroupBlock(lines, portGroups, indent = '') {
  lines.push(`${indent}$portGroups = @(`);
  for (const pg of portGroups) {
    lines.push(`${indent}    @{ Name = "${pg.name}"; VlanId = ${pg.vlan} }`);
  }
  lines.push(`${indent})`);
  lines.push('');
  lines.push(`${indent}foreach ($pg in $portGroups) {`);
  lines.push(`${indent}    if (-not (Get-VirtualPortGroup -VirtualSwitch $vSwitch -Name $pg.Name -ErrorAction SilentlyContinue)) {`);
  lines.push(`${indent}        New-VirtualPortGroup -VirtualSwitch $vSwitch -Name $pg.Name -VlanId $pg.VlanId | Out-Null`);
  lines.push(`${indent}    }`);
  lines.push(`${indent}    $pgObj = Get-VirtualPortGroup -VirtualSwitch $vSwitch -Name $pg.Name`);
  lines.push(`${indent}    Get-SecurityPolicy -VirtualPortGroup $pgObj |`);
  lines.push(`${indent}        Set-SecurityPolicy -AllowPromiscuous $true -ForgedTransmits $true -MacChanges $true | Out-Null`);
  lines.push(`${indent}    Write-Host "Port group ready: $($pg.Name) (VLAN $($pg.VlanId))"`);
  lines.push(`${indent}}`);
}
...
function emitVmCreationLoop(lines, hostNums, portGroups, nc, guestId, additionalDisks, sessionId) {
...
function buildDeployLabIso(spec, sessionId) {
  ...
  if (physCount === 1) {
    ...
    lines.push('');
    emitPortGroupBlock(lines, portGroups);
    lines.push('');
    emitVmCreationLoop(lines, h1HostNums, portGroups, nc, guestId, additionalDisks, sessionId);
    lines.push('Disconnect-VIServer -Server $VIServer -Confirm:$false');
  } else {
    ...
    lines.push('    }');
    lines.push('');
    lines.push('    # Port groups — must be created on every physical host');
    lines.push('    # Promiscuous mode and MAC changes must be on for nested ESXi frames to pass.');
    lines.push('');
    emitPortGroupBlock(lines, portGroups, '    ');
    lines.push('');
...
function buildDeployLabOva(spec) {
  ...
  if (portGroups.length === 0) portGroups.push({ name: 'Nested-Mgmt', vlan: 0 });

  const extraPortGroups = portGroups.filter((pg) => pg.name !== 'Nested-Mgmt');
  const additionalDisks = nc.additionalDisks || [];
  ...
  const emitOvaImport = (lines, indent = '') => {
    ...
    lines.push(`${indent}$ovfConfig.NetworkMapping.'VM Network'.Value         = "Nested-Mgmt"`);
    ...
    lines.push(`${indent}$ovfConfig.Common.guestinfo.vlan.Value                = "0"`);
    ...
    lines.push(`${indent}    -Location $VmFolder \``);
    ...
    if (extraPortGroups.length > 0) {
      lines.push('');
      lines.push(`${indent}# OVA has only the management NIC — add the rest before first boot`);
      for (const pg of extraPortGroups) {
        lines.push(`${indent}New-NetworkAdapter -VM $vm -NetworkName "${pg.name}" -Type Vmxnet3 -StartConnected | Out-Null`);
      }
    }
  ...
  if (physCountOva === 1) {
    ...
    lines.push('    }');
    lines.push('');
    lines.push('    # Port groups on this physical host');
    emitPortGroupBlock(lines, portGroups, '    ');
    lines.push('');
...
module.exports = { buildPowerShellScripts, buildRdpFile, buildLabConfigExample };
