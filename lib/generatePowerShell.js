...
function emitVmCreationLoop(lines, hostNums, portGroups, nc, guestId, additionalDisks) {
...
function buildDeployLabIso(spec) {
  ...
  if (physCount === 1) {
    ...
    lines.push('');
    emitVmCreationLoop(lines, h1HostNums, portGroups, nc, guestId, additionalDisks);
    lines.push('Disconnect-VIServer -Server $VIServer -Confirm:$false');
  } else {
    ...
    lines.push('');
    lines.push('        Get-CDDrive -VM $vm | Set-CDDrive -IsoPath $EsxiIsoPath -StartConnected $true -Confirm:$false | Out-Null');
    lines.push('        Write-Host "Created $vmName on $($physHost.IP). Power it on and run the ESXi installer."');
    lines.push('    }');
    lines.push('');
