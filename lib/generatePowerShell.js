...
function buildDeployLab(spec) {
  ...
  return isOva ? buildDeployLabOva(spec) : buildDeployLabIso(spec);
}

function buildDeployLabIso(spec) {
  const lines = [];
...
function buildDeployLabOva(spec) {
  ...
  lines.push('    }');
  lines.push('');
  lines.push('    Write-Host "Deploying $vmName ($hostIp) ..."');
  lines.push('');
  lines.push('    $ovfConfig = Get-OvfConfiguration -Ovf $EsxiOvaPath');
  lines.push('    $ovfConfig.NetworkMapping.\'VM Network\'.Value         = "Nested-Mgmt"');
  lines.push('    $ovfConfig.Common.guestinfo.hostname.Value            = $vmName');
  lines.push('    $ovfConfig.Common.guestinfo.ipaddress.Value           = $hostIp');
  lines.push('    $ovfConfig.Common.guestinfo.netmask.Value             = $MgmtSubnetMask');
  lines.push('    $ovfConfig.Common.guestinfo.gateway.Value             = $MgmtGateway');
  lines.push('    $ovfConfig.Common.guestinfo.vlan.Value                = "0"');
  lines.push('    $ovfConfig.Common.guestinfo.dns.Value                 = $MgmtDnsServer');
  lines.push('    $ovfConfig.Common.guestinfo.domain.Value              = $MgmtDomain');
  lines.push('    $ovfConfig.Common.guestinfo.ntp.Value                 = $NtpServer');
  lines.push('    $ovfConfig.Common.guestinfo.password.Value            = $plainPassword');
  lines.push('    $ovfConfig.Common.guestinfo.ssh.Value                 = "true"');
  lines.push('    $ovfConfig.Common.guestinfo.createvmfs.Value          = "false"');
  lines.push('');
  lines.push('    $vm = Import-VApp -Source $EsxiOvaPath `');
  ...
  if (extraPortGroups.length > 0) {
    lines.push('    # OVA has only the management NIC — add the rest before first boot');
  ...
  if (additionalDisks.length > 0) {
    lines.push('    # Additional virtual disks (vSAN capacity, local datastore, etc.)');
    lines.push('    foreach ($disk in $additionalDisks) {');
    lines.push("        if ($disk.Purpose -eq 'local_datastore' -and $hostNum -ne 1) { continue }");
    lines.push('        New-HardDisk -VM $vm -CapacityGB $disk.SizeGB -StorageFormat Thin -Confirm:$false | Out-Null');
    lines.push('    }');
    lines.push('');
  }
  lines.push('    Start-VM -VM $vm | Out-Null');
  lines.push('    Write-Host "  Started $vmName — ESXi booting (allow 2-3 minutes for first boot)"');
  lines.push('}');
  lines.push('');
  lines.push('Write-Host ""');
