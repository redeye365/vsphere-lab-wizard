...
function buildMarkdown(spec) {
  ...
  if (spec.architectMode && spec.learningMode) {
    ...
    const openItems = [];
    if (!disc.problemStatement && !dr.successStatement) openItems.push('Problem statement not captured');
    const highRisks = risks.filter(r => r.likelihood === 'high' && r.impact === 'high' && !r.mitigation);
    if (highRisks.length) openItems.push(`${highRisks.length} high-severity risk(s) without mitigation`);
    ...
    if (missing.length) openItems.push(`Decision rationale missing for: ${missing.join(', ')}`);
    if (openItems.length) {
      out.push('## Open Items');
      out.push('');
  ...
  // --- Resource summary (traffic-light) ---
  {
    ...
    if (dc.enabled)   rows.push({ name: 'Domain controller',        ram: 4 });
    if (nsx.enabled) {
