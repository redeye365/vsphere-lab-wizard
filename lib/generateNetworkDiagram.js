...
function buildMermaidDiagram(spec) {
  ...
  const renderPhysSubgraph = (physIdx) => {
    ...
    if (phSpec.cpuCores) physParts.push(`${phSpec.cpuCores} cores`);
    if (phSpec.ramGB)    physParts.push(`${phSpec.ramGB} GB RAM`);
    if (phSpec.ipAddress) physParts.push(phSpec.ipAddress);
