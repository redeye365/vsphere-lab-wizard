...
function buildMermaidDiagram(spec) {
  ...
  // Direct physical-layer VMs subgraph (DC, jumpbox — run on the physical host, not inside nested ESXi)
  ...
  // Nested-layer subgraph: ESXi host VMs + the VMs that run inside them (vCenter, NSX)
  const hostCount  = nc.hostCount || 0;
  const shownHosts = Math.min(hostCount, 6);
  lines.push('        subgraph NESTED["Nested lab · VMs deployed inside nested ESXi hosts"]');
